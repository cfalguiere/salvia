#!/bin/bash

set -eu
#set -x

envname=labbench

region=$(aws ec2 describe-availability-zones --output text --query 'AvailabilityZones[0].[RegionName]')
[ -z "$region" ] && (echo "ERROR: region is empty"; exit 1)

account=$(aws sts get-caller-identity --output text --query 'Account')
[ -z "account" ] && (echo "ERROR: account is empty"; exit 1)

states_bucket="terraform-${region}-${account}"
if [[ ! -z $(aws s3api head-bucket --bucket $states_bucket 2>&1) ]]; then
    # got an error when trying to access the bucket
    echo "Creating terraform states bucket"
    aws s3api create-bucket \
    --bucket $states_bucket \
    --region $region \
    --create-bucket-configuration LocationConstraint=$region
fi

echo "Terraform S3 states buccket is $states_bucket"

#credentials_file_name=$(find ~/.aws -name "credentials")

backend_config_file_name="terraform/var-backend.generated.auto.tfvars"
run_config_file_name="terraform/var.generated.auto.tfvars"

cat > $backend_config_file_name <<TEMPLATE1
# Generated by terreform-configure.sh 
# DO not edit

# Backend access configuration
# use with -var-file
#
# This part also act as a backend configuration file
# Do not change the key names
# use with -backend-config in init
#

# make sure the bucket exists
# make user or role has access to the bucket and files
bucket = "$states_bucket"

# path to the state file inside the bucket
key = "states/salvia/stages/env-${envname}.state"

# do not encrypt the bucket
encrypt = false

# choose a region
region = "$region"
TEMPLATE1

echo "file $backend_config_file_name created"


cat > $run_config_file_name <<TEMPLATE2
# Generated by terreform-configure.sh 
# DO not edit

# choose a platform name for tags
platform = "salvia"
platform_label = "Salvia"

# choose en environment name for tags
environment = "labbench"
environment_label = "LabBench"

TEMPLATE2

echo "file $run_config_file_name created"



