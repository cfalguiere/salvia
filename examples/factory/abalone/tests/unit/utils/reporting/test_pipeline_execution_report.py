...
def test_create_combined_json_report(json_report_with_fillers):
    ...
    for section_name in [
        ...
        expected_error_message = (
            reference[0][error_key] if section_name in ["execution_steps", "lineage"] else reference["error_message"]
        )

        assert error_key in section
        assert section[error_key] == expected_error_message
...
# @pytest.mark.xfail
def test_generate_report_from_combined_json(sample_combined_json_report, report_generator):

    with tempfile.TemporaryDirectory() as tmpdirname:
        report_settings = PipelineExecutionReportSettings()
        report_settings.report_folder = tmpdirname
        PipelineExecutionReport(settings=report_settings).generate_report_from_combined_json(
        ...
        assert "| PipelineExecutionStatus | Succeeded |" in lines[10]
...
def test_show_step_failure(failed_a_step_json_report):
    ...
    # line feeds are stripped out so that they do not break the table
    expected_message = execution_steps[0]["FailureReason"].replace("\n", " ").replace("'", "&#39;")
    assert expected_message in md_report
...
# @pytest.mark.xfail
def test_table_steps_formatting(steps_table_one_step_json_report, steps_table_two_steps_json_report):
...
# @pytest.mark.xfail
def test_table_lineage_formatting(lineage_table_one_line_json_report, lineage_table_two_lines_json_report):
    # ensure table lines are have correct line split and no extra lines
    report_generator = PipelineExecutionReport()
    ...
    line = "| s3://...022-12-02-21-04-06-122/output/validation | Input | DataSet | ContributedTo | artifact |"
    assert line in md_report_1


# @pytest.mark.xfail
def test_table_metrics_formatting(regression_one_metric_json_report, regression_json_report):
    # ensure table lines are have correct line split and no extra lines
    report_generator = PipelineExecutionReport()
...
@pytest.mark.xfail
def test_table_parameters_formatting(failed_a_step_json_report, regression_json_report):
    # ensure table lines are have correct line split and no extra lines
    pass  # TODO
...
def test_no_parameters_formatting(definition_parameters_json_report):
    ...
    md_report = PipelineExecutionReport().create_markdown_report(definition_parameters_json_report)
...
def test_default_parameters_formatting(default_parameters_json_report):
    ...
    md_report = PipelineExecutionReport().create_markdown_report(default_parameters_json_report)
...
def test_custom_parameters_formatting(mixed_parameters_json_report):
    ...
    md_report = PipelineExecutionReport().create_markdown_report(mixed_parameters_json_report)
