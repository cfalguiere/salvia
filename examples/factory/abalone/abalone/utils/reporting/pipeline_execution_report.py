"""This module contains helper function to create pipeline's run summaries.
"""
import json
import logging
import traceback
...
def create_combined_json_report(sagemaker, pipeline, execution, eval_file_uri) -> dict:
    ...
    """
    definition_report = _get_pipeline_definition(pipeline)

    execution_definition_report = _get_execution_definition(execution)

    execution_steps_report = _get_execution_steps(execution)

    lineage_report = _get_lineage(sagemaker, execution_steps_report)

    evaluation_report = _get_evaluation(sagemaker, eval_file_uri)
    ...
    return _enhance_json(combined_report)


def write_json_report(filepath: str, json_report: dict) -> None:
...
def create_markdown_report(json_report: dict, 
...
def write_markdown_report(filepath: str, content: str) -> None:
...
def _fallback_report_data(exc, message: str) -> Dict[str, Any]:
...
def _get_pipeline_definition(pipeline) -> Dict[str, Any]:
    ...
    try:
        ...
        definition_report = _fallback_report_data(exc, "Could not read pipeline definitions")
    return definition_report


def _get_execution_definition(execution) -> Dict[str, Any]:
    ...
    try:
        ...
        execution_definition_report = _fallback_report_data(exc, "Could not read execution definition")
    return execution_definition_report


def _get_execution_steps(execution) -> List[Dict[str, Any]]:
    # list the steps in the execution as a list of dicts
    logging.info("fetching steps")
    try:
        execution_steps_report = execution.list_steps()
    except (AttributeError, TypeError, ValueError) as exc:
        error_status = _fallback_report_data(exc, "Could not read execution steps")
        execution_steps_report = [error_status]
    return execution_steps_report


def _get_lineage(sagemaker, execution_steps_report) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        error_status = _fallback_report_data(exc, "Could not read lineage")
        lineage_report = [error_status]
    return lineage_report


def _get_evaluation(sagemaker, eval_file_uri) -> Dict[str, Any]:
    ...
    try:
        ...
        evaluation_report = _fallback_report_data(exc, "Could not read evaluation report")
    return evaluation_report


def _enhance_json(json_report: dict) -> Dict[str, Any]:
